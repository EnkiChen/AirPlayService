...
- (BOOL)setPrivateKey
{
	...
	[keyAttr setObject:key forKey:(__bridge id)kSecValueData];
	[keyAttr setObject:[NSNumber numberWithBool:YES] forKey:(__bridge id)kSecReturnPersistentRef];

	SecKeyRef persistPrivateKey = NULL;
	
	status = SecItemAdd((__bridge CFDictionaryRef) keyAttr, (CFTypeRef *) &persistPrivateKey);
	if (status != noErr) {
		NSLog(@"[Crypto] SecItemAdd error %d", (int)status);
		return NO;
	}
	...
	if (persistPrivateKey) {
        ...
        status = SecItemCopyMatching((__bridge CFDictionaryRef) keyAttr, (CFTypeRef *) &privateKey);
		if (status != noErr) {
			NSLog(@"[Crypto] SecItemCopyMatching error %d", (int)status);
			return NO;
		}
    }
...
- (NSData *)challengeResponse:(NSData *)challenge withAddr:(NSData *)addr
{
	...
	if (status != noErr) {
		NSLog(@"[Crypto] SecKeyRawSign error %d", (int)status);
		return nil;
	}
...
- (NSData *)decryptRsa:(NSData *)data
{
	...
	if (status != noErr) {
		NSLog(@"[Crypto] SecKeyDecrypt error %d", (int)status);
		free(plainText);
		return nil;
	}
