...
@interface AirPlayService () <NSNetServiceDelegate>
{
    NSNetService		*netService;
    NSNetService		*airPlayService;
    
    GCDAsyncSocket		*asyncSocket;
    GCDAsyncSocket		*airplaySocket;
...
@property (nonatomic, copy) NSString *servieName;
...
{
    [self setupSocket];
    self.servieName = serviceName;
    [self publishRaopService];
    [self publishAirPlayService];
}

- (void) publishAirPlayService
{
    UInt16 port = [airplaySocket localPort];
    if ( nil == airPlayService ) {
        airPlayService = [[NSNetService alloc] initWithDomain:@"local."
                                                         type:@"_airplay._tcp."
                                                         name:self.servieName
                                                         port:port];
    }
    ...
    NSDictionary *txtDict = @{
                              @"rhd":@"0.0.1",
                              @"deviceid":[DeviceInfo deviceIdWithSep:@":"],
                              @"features":@"0x100029ff",
                              @"model":@"AppleTV3,2",
                              @"vv":@"1",
                              @"srcvers":@"150.33",
    ...
    [airPlayService setTXTRecordData:txtData];
    
    [airPlayService setDelegate:self];
    [airPlayService publish];
}

- (void) publishRaopService
{
...
- (BOOL) setupSocket
{
    ...
    airplaySocket = [[GCDAsyncSocket alloc] initWithDelegate:self delegateQueue:dispatch_get_main_queue()];
    ...
    if (![airplaySocket acceptOnPort:0 error:&err])
    {
        NSLog(@"Error in acceptOnPort:error: -> %@", err);
        return NO;
    }
    ...
    clientLists = [[NSMutableArray alloc] init];
...
{
    // 保存新的客户端
    [clientLists addObject:newSocket];
    
    NSLog(@"Accept = %@ %@", newSocket, airplaySocket);
    NSLog(@"[Net] accept connection from %@:%hu to %@:%hu", [newSocket connectedHost], [newSocket connectedPort], [newSocket localHost], [newSocket localPort]);
       ...
       withData:(NSData *)data
{
    ...
    if ([data length] > 0) {
        ...
        [str appendFormat:@"Content-Length: %lu\r\n", [data length]];
    }
...
- (void)socketMirroringReceivedRequest:(GCDAsyncSocket *)sock
{
...
- (void)socketReceivedRequest:(GCDAsyncSocket *)sock
{
        ...
        [location isEqualToString:@"/fp-setup"])
    {
        ...
        if (fply_header[6] == 1) {
            ...
            data = [NSMutableData dataWithBytesNoCopy:fply_2 length:sizeof(fply_2) freeWhenDone:NO];
    ...
    } else  if ( [method isEqualToString:@"GET_PARAMETER"] ) {
        ...
        if ( [content_type isEqualToString:@"text/parameters"] ) {
            ...
            for (NSString *param in [parameters componentsSeparatedByString:@"\r\n"]) {
                ...
                NSString *contentString = @"volume: 0.000000";
                
                NSArray *a = [param componentsSeparatedByString:@":"];
                NSString *key = [a objectAtIndex:0];
                
                if ( [key isEqualToString:@"volume"] )
                    [self replyOK:sock withHeaders:nil withData:[contentString dataUsingEncoding:NSASCIIStringEncoding]];
            }
...
{
    NSLog(@"[Bonjour] service published: domain(%@) type(%@) name(%@) port(%ld)",
          [ns domain], [ns type], [ns name], (long)[ns port]);
}
